...
func UpdateCmd(ctx *telegohandler.Context, message telego.Message) error {
	...
	reply, _ := utils.ReplyMessageWithHTML(ctx, message, fmt.Sprintf(
		"发现新版本 <code>%s</code> (当前 <code>%s</code>)\n安装包: <code>%s</code> (%.1f MB)\n\n确认后会自动下载并重启, 旧版本会归档为 LotsACG.old.exe, 更新失败时自动回退。",
		latestName, version.Version, asset.Name, sizeMB,
	))
	if reply == nil {
		return nil
	}
...
func UpdateConfirmCallback(ctx *telegohandler.Context, query telego.CallbackQuery) error {
	...
	newPath := filepath.Join(exeDir, "LotsACG_new.exe")
	if err := downloadToFile(ctx, asset.BrowserDownloadURL, newPath, asset.Size); err != nil {
		_ = os.Remove(newPath)
	...
	if runtime.GOOS == "windows" {
		if err := writeUpdateBat(exeDir); err != nil {
...
// downloadToFile 下载文件到指定路径, 并校验大小。
func downloadToFile(ctx context.Context, downloadURL, dest string, expectedSize int64) error {
	...
	if err != nil {
		return oops.Wrapf(err, "write file failed")
	}
	// 校验: 下载内容不能太小 (避免下载到错误页面), 且尽量接近期望大小
	if n < 1024*1024 {
		return oops.Errorf("downloaded file too small (%d bytes)", n)
	}
	if expectedSize > 0 && n < expectedSize/2 {
		return oops.Errorf("downloaded file size mismatch (got %d, want %d)", n, expectedSize)
	}
	return nil
}

// writeUpdateBat 生成 Windows 更新脚本: 等旧进程退出 → 归档旧 exe → 替换 → 启动新 exe。
func writeUpdateBat(exeDir string) error {
	bat := `@echo off
chcp 65001 >nul
timeout /t 3 /nobreak >nul
:kill
taskkill /IM LotsACG.exe /F >nul 2>&1
timeout /t 1 /nobreak >nul
if exist LotsACG.old.exe del /F LotsACG.old.exe
move /Y LotsACG.exe LotsACG.old.exe >nul 2>&1
if errorlevel 1 goto kill
move /Y LotsACG_new.exe LotsACG.exe >nul 2>&1
...
exit /b 0
:restore
move /Y LotsACG.old.exe LotsACG.exe >nul 2>&1
echo [LotsACG] update failed, restored old version (LotsACG.old.exe)
del "%~f0"
exit /b 1
`
	// bat 需要 CRLF 换行
